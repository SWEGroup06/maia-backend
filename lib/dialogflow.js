...
const context = {
  _actionHandlers: {
    ...
    tp: function (fields) {
      ...
      const newDateRange = context._parseDateRange(
        fields["new-specific-date"],
        fields["new-date-range"]
      );
      const newTimeRange = context._parseTimeRange(
  ...
  parseScheduleRequest(time) {
    ...
    if (time) {
      if (
        time["very-specific-meeting-time"].structValue &&
        time["very-specific-meeting-time"].structValue.fields
      ) {
        ...
        if (time["very-specific-meeting-time"].structValue.fields.date_time) {
          ...
          res.time = [
            time["very-specific-meeting-time"].structValue.fields.date_time
              .stringValue,
          ];
          ...
          startTime =
            time["very-specific-meeting-time"].structValue.fields.startDateTime
              .stringValue;
          endTime =
            time["very-specific-meeting-time"].structValue.fields.endDateTime
              .stringValue;
        ...
        startTime = DateTime.fromISO(
          time["very-specific-meeting-time"].stringValue,
          { zone: "Europe/Paris" }
        ).toISO();
        endTime = DateTime.fromISO(startTime, { zone: "Europe/Paris" })
          .plus({ hours: 1 })
          .toISO();
        ...
        startTime = DateTime.fromISO(
          time["specific-meeting-time"].stringValue
        ).startOf("day");
        endTime = startTime.endOf("day");

        res.time = [startTime.toISO(), endTime.toISO()];
      } else if (
        time["meeting-time"].structValue &&
        time["meeting-time"].structValue.fields
      ) {
        /* Where a range of dates is given, e.g. a whole week, month, etc.
         */

        startTime =
          time["meeting-time"].structValue.fields.startDate.stringValue;
  ...
  _parseDateRange(specificDate, dateRange) {
    ...
    const newDateRange = {
      start: TODAY.toISO(),
      end: TODAY.plus({ months: 1 }).toISO(),
    };

    if (specificDate.stringValue !== "") {
      newDateRange.start = DateTime.fromISO(specificDate.stringValue)
        .startOf("day")
        .toISO();
      newDateRange.end = DateTime.fromISO(specificDate.stringValue)
        .endOf("day")
        .toISO();
      return newDateRange;
    }
  ...
  _parseTimeRange(specificTime, timeRange) {
    ...
    const newTimeRange = {
      start: CURR_DATE_TIME.startOf("day").toISO(),
