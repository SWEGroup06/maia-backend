...
const context = {
    ...
    if (time) {
      ...
      if (time['very-specific-meeting-time'].structValue && time['very-specific-meeting-time'].structValue.fields) {
        ...
        if (time['very-specific-meeting-time'].structValue.fields.date_time) {
          // this is the case when only a specific time is given
          ...
          res.time = [time['very-specific-meeting-time'].structValue.fields.startDateTime.stringValue,
            time['very-specific-meeting-time'].structValue.fields.endDateTime.stringValue];
        }
   ...
   * @return {int} returns an integer to indicate the category (work = 1, social = 0, unknown = -1).
   */
  async getCategory(title) {
    ...
    if (responses && responses.length && responses[0]) {
      ...
      if (queryRes.action == 'unknown') return -1;
      return queryRes.action == 'work' ? 1 : 0;
    } else {
