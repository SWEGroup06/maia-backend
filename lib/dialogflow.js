...
const context = {
  _actionHandlers: {
    ...
    tp: function(fields) {
      ...
      return ({
        oldTitle: fields['old-title'],
        oldDateTime: TIME.combineDateAndTime(fields['old-date'], fields['old-time']),
  ...
  _parseDateRange(specificDate, dateRange) {
    ...
    if (specificDate.stringValue !== '') {
      newDateRange.start = DateTime.fromISO(specificDate.stringValue).startOf('day');
      newDateRange.end = DateTime.fromISO(specificDate.stringValue).endOf('day');
      return newDateRange;
    }
