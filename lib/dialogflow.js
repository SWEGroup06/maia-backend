...
const context = {
  _actionHandlers: {
    ...
    sp: function(fields) {
      ...
      const dateProperties = context._parseDateRange(fields['specific-date'], fields['date-range'], 'schedule');
      const timeProperties = context._parseTimeRange(fields['specific-time'], fields['time-range'], 'schedule');
    ...
    tp: function(fields) {
      ...
      const dateProperties = context._parseDateRange(fields['new-specific-date'], fields['new-date-range'], 'reschedule');
      const timeProperties = context._parseTimeRange(fields['new-specific-time'], fields['new-time-range'], 'reschedule');
   ...
   * @private
   */
  _parseDateRange(specificDate, dateRangeObj, command) {
    const dateRange = {start: null, end: null};
    let flexible = true;

    if (command !== 'reschedule') {
      // If this function is used to reschedule, we want to use the Maia-managed event's current date range
      // therefore, not setting a default one as below.
      const TODAY = DateTime.local();
      dateRange.start = TODAY.toISO();
   ...
   * @private
   */
  _parseTimeRange(specificTime, timeRangeObj, command) {
    const timeRange = {start: null, end: null};
    let flexible = true;

    if (command !== 'reschedule') {
      // If this function is used to reschedule, we want to use the Maia-managed event's current time range
      // therefore, not setting a default one as below.
      const TODAY = DateTime.local();
