    ...
    transporter.sendMail(
      {
        ...
        html: suggestions
          .map(
            (
              meetingData
            ) => `<div style='color: #000;font-size: 15pt;font-weight: bold;margin-bottom: 5px;margin-top: 20px;'>${
              meetingData.name
            }</div>
      <div style='color: #000;font-size: 12pt;'><span style='font-weight:bold;margin-right: 10px'>FROM</span> ${new Date(
        meetingData.from.start
      ).toGMTString()} - ${new Date(
              meetingData.from.end
            ).toGMTString()}</h3></div>
      <div style='color: #000;font-size: 12pt;'><span style='font-weight:bold;margin-right: 10px'>TO</span> ${new Date(
        meetingData.to.start
      ).toGMTString()} - ${new Date(
              meetingData.to.end
            ).toGMTString()}</h3></div>
      <div style='margin-top: 20px'><a href='${endPoint}/auto/accept?email=${encodeURIComponent(
