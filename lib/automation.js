      ...
      for (const user of allUserData) {
        ...
        for (const event of maiaManagedEvents) {
          ...
          if (Math.abs(changeDifference.milliseconds) > THRESHOLD) {
            ...
            busyTimes = await GOOGLE.getBusySchedule(token, today.toISO(), nextWeek.toISO());
            freeTimes = await SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), today.toISO(), nextWeek.toISO());
          }
        }
        // console.log('\n\n*****updationsMade****\n', updationsMade);
