  ...
  start: async function(INTERVAL) {
    const update = async function() {
      ...
      for (const user of allUserData) {
        ...
        const res = await fetch('https://www.googleapis.com/calendar/v3/calendars/kpal81xd@gmail.com/events/watch', {
          method: 'POST',
          headers: {
          ...
          body: {
            id: 123,
            type: 'web_hook',
            address: 'http://09e0034d308f.ngrok.io',
          }
        });
        ...
        for (const meeting of meetings) {
          ...
          const category = DIALOGFLOW.getCategory(meeting.summary);
          // Determine new meeting slot and calculate time difference between original slot
          const chosenSlot = SCHEDULER.findMeetingSlot([freeTimes], duration, [constraints], [history], category);
