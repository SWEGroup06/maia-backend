  ...
  start: async function(INTERVAL) {
    const update = async function() {
      ...
      for (const user of allUserData) {
        ...
        let freeTimes = SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), today.toISO(), nextWeek.toISO(), minBreakLength);
        const updationsMade = [];
        ...
        for (const event of maiaManagedEvents) {
          ...
          const chosenSlot = SCHEDULER.findMeetingSlot([freeTimes], duration, [constraints],
              [histFreqsPerCat[category + 1]], category);
