  ...
  start: async function(INTERVAL) {
    const update = async function() {
      ...
      for (const user of allUserData) {
        ...
        let lastMonthHist = await GOOGLE.getMeetings(token, oneMonthAgo.toISO(), _today.toISO());
        ...
        for (let i = 0; i < user.frequencies.length; i++) {
          ...
          while (histFreqInfo.timestamp == null) {
            ...
            await delay(30000);
            histFreqInfo = await DATABASE.getFrequenciesForCategoryFromGoogleEmail(email, category);
          }
          let histFreq = histFreqInfo.histFreq;
          if (_today.diff(DateTime.fromISO(histFreqInfo.timestamp), ['minutes']) > histFreqLifetime) {
            console.log('UPDATING HISTORY FREQUENCIES');
            // if out of date, recalculate hist freq and update in db
            histFreq = await SCHEDULER.generateUserHistory(categorisedSchedule, i);
            await DATABASE.setFrequenciesForCategoryFromGoogleEmail(user.google.email, i, histFreq);
          }
        ...
        let busyTimes = await GOOGLE.getBusySchedule(token, today.toISO(), nextWeek.toISO());
        let freeTimes = SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), today.toISO(), nextWeek.toISO(), minBreakLength, workingHours);
        const updationsMade = [];
        ...
        for (const event of maiaManagedEvents) {
          ...
          busyTimes = await GOOGLE.getBusySchedule(token, event.startDateRange, event.endDateRange);
          freeTimes = await SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), event.startDateRange, event.endDateRange, minBreakLength);

          const chosenSlot = SCHEDULER.findMeetingSlot([freeTimes], duration, [histFreqsPerCat[category + 1]], category);
          ...
          if (Math.abs(changeDifference.milliseconds) > THRESHOLD) {
            ...
            await GOOGLE.updateMeeting(token, {
              id: event.id, summary: event.title, description: event.description,
              attendees: event.attendees,
            }, chosenSlot.start, chosenSlot.end);
            updationsMade.push({
              name: event.title,
