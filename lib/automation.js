  ...
  start: async function(INTERVAL) {
    const update = async function() {
      ...
      for (const user of allUserData) {
        ...
        let freeTimes = SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), today.toISO(), nextWeek.toISO());
        ...
        for (const event of maiaManagedEvents) {
          ...
          freeTimes = await SCHEDULER.getFreeSlots(busyTimes.map((slot) => [slot.start, slot.end]), event.startDateRange, event.endDateRange);
