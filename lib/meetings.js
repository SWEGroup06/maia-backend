...
module.exports = {
  ...
  async generateHistFreqForCategory(categorisedSchedule, slackEmail, category) {
  ...
  async generatePreferences(email) {
    if (!await DATABASE.userExists(email)) {
      throw new Error(`${email} is not signed in`);
    }
    ...
    console.log('here');
    let lastMonthHist = await GOOGLE.getMeetings(token, ONEMONTHAGO.toISO(), TODAY.toISO());
    lastMonthHist = lastMonthHist.map((e) => [e.start.dateTime, e.end.dateTime, e.summary]);
    const categorisedSchedule = await SCHEDULER.getCategorisedSchedule(lastMonthHist);
    ...
    for (let category=0; category < NUM_CATEGORIES; category++) {
      ...
      this.generateHistFreqForCategory(categorisedSchedule, email, category);
    }
  },
  /**
   * TODO: Comment
  ...
  async schedule(title, slackEmails, startDateRangeISO, endDateRangeISO, flexible,
      duration) {
    const freeTimes = [];
    const constraints = [];
    const histFreqs = [];
    ...
    for (const email of slackEmails) {
      ...
      if (flexible) {
        ...
        const minBreakLength = Duration.fromObject({minutes: 5});
