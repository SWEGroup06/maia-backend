...
module.exports = {
   ...
   */
  async schedule(title, slackEmails, startDateRange, endDateRange,
      flexible, beforeAfterKey) {
    const busyTimes = [];
    const constraints = [];
    const histFreqs = [];
    let category;
    ...
    for (const email of slackEmails) {
      ...
      if (histFreqInfo.timestamp == null || today.diff(DateTime.fromISO(histFreqInfo.timestamp),
          ['minutes']) > histFreqLifetime) {
        ...
        histFreq = await SCHEDULER.getUserHistory(
            lastMonthHist.map((e) => [e.start.dateTime, e.end.dateTime, e.summary]), category);
  ...
  async reschedule(eventStartTime, title, organiserSlackEmail, startOfRangeToRescheduleTo,
    ...
    const updatedDescription = JSON.parse(descriptions[0].description);
    ...
    for (const email of attendeeEmails) {
      ...
      const histFreqInfo = await DATABASE.getFrequenciesForCategoryFromID(email, category);
      let histFreq = histFreqInfo.histFreq;
      if (!histFreq || !histFreqInfo.timestamp || today.diff(DateTime.fromISO(histFreqInfo.timestamp),
          ['minutes']) > histFreqLifetime) {
        // if out of date, recalculate hist freq and update in db
        const lastMonthHist = await GOOGLE.getMeetings(token, oneMonthAgo.toISO(), today.toISO());
        histFreq = await SCHEDULER.getUserHistory(
