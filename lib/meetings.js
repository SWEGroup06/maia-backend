...
const { getDescription } = require('./google.js');
const { start } = require('./automation.js');
...
module.exports = {
  ...
  async schedule(title, slackEmails, startDate, endDate, flexible) {
    const busyTimes = [];
    const constraints = [];
    const lastMonthHistories = [];

    // TODO: Stringify this and send it into the description
    const description = {flexible: flexible, startDate: startDate, endDate: endDate};
    ...
    for (const email of slackEmails) {
      ...
      // console.log('events: ', lastMonthHist);
      if (lastMonthHist) lastMonthHistories.push(lastMonthHist.map((e) => [e.start.dateTime, e.end.dateTime, e.summary]));
    }
    ...
    // TODO: change to event name inputted by user!
    const eventName = 'meeting';
    const category = await DIALOGFLOW.getCategory(eventName);
    // Using free times find a meeting slot and get the choice
    const chosenSlot = SCHEDULER.findMeetingSlot(freeTimes, eventDuration, constraints, lastMonthHistories, category);
    if (!chosenSlot) {
 ...
 */
  ...
  async getDescriptions(email, startDate, endDate) {
    try {
    // Check if a user with the provided details existing in the database
      if (!await DATABASE.userExists(email)) {
        console.log('error: '+ email + ' is not signed in');
      ...
      // Get tokens from the database
      const token = JSON.parse(await DATABASE.getToken(email));
      const events = await GOOGLE.getMeetings(token, startDate, endDate);
      if (!events || events.length === 0) {
        console.log('No event found in time frame');
        return;
      }
      ...
      events.forEach(element => {
        ...
        console.log("event ID: " + element.id)
        try {
          let description = GOOGLE.getDescription(token, element.id);
          console.log(description);
          // if (description.flexible && description.startDate && description.endDate){
          //   eventsWithDesc.push([element, flexible, startDate, endDate]);
          //   console.log("yes")
          // }
        } catch (err) {
          console.log("no")
      ...
      console.log(eventsWithDesc);

      
      const eventDict = eventsWithDesc.map((event) => [event[0].summary, event[0].start.dateTime, event[0].end.dateTime, event[0], event[1], event[2]]);
      return eventDict;
    } catch (error) {
      console.error(error);
    }
  ...
  async reschedule(eventStartTime, meetingTitle, organiserSlackEmail, startOfRangeToRescheduleTo, endOfRangeToRescheduleTo) {
    ...
    const eventName = originalEvent.summary;
    ...
    // Get free slots from the provided busy times
    const freeTimes = busyTimes.map((timeSlot) => SCHEDULER.getFreeSlots(timeSlot, startDate, endDate));
    const category = DIALOGFLOW.getCategory(eventName);
    // Using free times find a meeting slot and get the choice
    const chosenSlot = SCHEDULER.findMeetingSlot(freeTimes, eventDuration, constraints, lastMonthHistories, category);
