...
module.exports = {
  ...
  async schedule(title, slackEmails, startDateRange, endDateRange, flexible) {
    ...
    for (const email of slackEmails) {
      ...
      while (histFreqInfo.timestamp == null && tried < 2) {
        console.log(histFreqInfo.timestamp, category, email);
        tried++;
  ...
  async rescheduleSpecific(email, name, date, startTime, endTime) {
    const token = JSON.parse(await DATABASE.getToken(email));
    const event = soimethign;

    const startTime24 = startTime.toLocaleString(DateTime.TIME_24_SIMPLE);
    const endTime24 = endTime.toLocaleString(DateTime.TIME_24_SIMPLE);
    await GOOGLE.updateMeeting(token, event, startDateTime, endDateTime);
  },
