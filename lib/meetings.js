...
const context = {
  ...
  async sp(googleEmails, title, duration, startDateRangeISO, endDateRangeISO, startTimeRangeISO, endTimeRangeISO,
      flexible, dayOfWeek, timeRangeSpecified) {
    const freeTimes = [];
    const constraints = [];
    const histFreqs = [];
    const tokens = [];
    ...
    let chosenSlot;
    if (flexible) {
      chosenSlot = await SCHEDULER.findMeetingSlot(freeTimes, eventDuration, constraints, histFreqs, constraintsSet);
    } else {
