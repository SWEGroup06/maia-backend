...
const context = {
  ...
  async schedule(title, googleEmails, startDateRangeISO, endDateRangeISO, flexible, duration) {
    ...
    let chosenSlot;
    if (flexible) {
      chosenSlot = await SCHEDULER.findMeetingSlot(freeTimes, eventDuration, constraints, histFreqs, category);
    } else {
      chosenSlot = {
  ...
  async tp(
    ...
    const chosenSlot = await SCHEDULER.findMeetingSlot(freeTimes, eventDuration, null, histFreqs, category);
  ...
  async reschedule(eventStartTimeISO, title, organiserGoogleEmail, startOfRangeToRescheduleToISO,
    ...
    // Using free times find a meeting slot and get the choice
    const chosenSlot = await SCHEDULER.findMeetingSlot(freeTimes, eventDuration, constraints, histFreqs, category);
    if (!chosenSlot) {
      throw new Error('Slot not found');
    }
