...
module.exports = {
   ...
   * @param {Array} days constrained days.
   * @param {Object} times constrained times.
   * @return {number} A list of meetings for the following week.
   */
  async setConstraints(email, days, times) {
 ...
 */
   ...
   * @return {Promise<undefined|{start, end}|null>}
   */
  async reschedule(eventStartTime, eventTitle, organiserSlackEmail, startOfRangeToRescheduleTo, endOfRangeToRescheduleTo) {
    const constraints = [];
    ...
    if (!eventTitle || eventTitle === '') {
      ...
      category = await DIALOGFLOW.getCategory(eventTitle);
    }
    ...
    if (eventStartTime) {
      ...
      events = await GOOGLE.getEventByName(organiserToken, eventTitle, today.toISO(), oneMonthLater.toISO());
    }
