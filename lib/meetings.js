...
const context = {
  ...
  async getHistFreq(token, category, googleEmail) {
    let histFreqInfo = await DATABASE.getFrequenciesForCategoryFromGoogleEmail(googleEmail, category);
    let tried = 0;
    // TODO: Fix this wtf
    while (histFreqInfo.timestamp == null && tried < 2) {
      tried++;
      console.log('HIST FREQ FETCH:', tried);
      // wait for 5 seconds then try again
      await delay(3000);
      histFreqInfo = await DATABASE.getFrequenciesForCategoryFromGoogleEmail(googleEmail, category);
    }
    if (!histFreqInfo.timestamp == null) return null;
  ...
  async tp(googleEmail, oldTitle, oldDateTimeISO, newStartDateRangeISO, newEndDateRangeISO,
    ...
    // Update description
    const description = JSON.parse(await GOOGLE.getDescription(organiserToken, oldEvent.id));
    ...
    const eventDuration = eventStartDateTime.diff(eventEndDateTime);

    /**
     * TODO: This is the function we will use to generate time ranges in
     * TODO: the format [[], ..., [{'start': 12pm, 'end': 8pm}], [{'start': 12pm, 'end': 8pm}]]
     *
     * TODO: Amelia + Hasan this is the magic thing you use!!!!!!!!
     */
    // eslint-disable-next-line no-unused-vars
    const timeRangesForDaysOfWeek =
     TIME.generateTimeRangesForDaysOfWeek(newDayOfWeek, newStartTimeRangeISO, newEndTimeRangeISO);
    ...
    for (const attendeeGoogleEmail of attendeeGoogleEmails) {
      ...
      const attendeeConstraints = await DATABASE.getConstraintsFromGoogleEmail(attendeeGoogleEmail);

      const generatedConstraints =
       SCHEDULER.generateConstraints(attendeeConstraints, newStartDateRangeISO, newEndDateRangeISO);

      if (generatedConstraints && generatedConstraints.length !== 0) {
        constraints.push(generatedConstraints);
      }
      ...
      if (busyTimes) {
        freeTimes.push(SCHEDULER.getFreeSlots((busyTimes.map((e) => [e.start, e.end])), newStartDateRangeISO,
            newEndDateRangeISO, minBreakLength));
      }
  ...
  async generatePreferences(googleEmail, tokens) {
    ...
    for (let category=0; category < NUM_CATEGORIES; category++) {
      module.exports.generateHistFreqForCategory(categorisedSchedule, googleEmail, category);
    }
    // TODO: ...
  },
