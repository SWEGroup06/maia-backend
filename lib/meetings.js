...
const context = {
  ...
  async sp(
    ...
    const description = {
      ...
      timeRangesForDaysOfWeek: timeRangeSpecified ? timeRangesForDaysOfWeek : [],
    };
  ...
  async tp(
    ...
    const category = await DIALOGFLOW.getCategory(events[0].summary) === 0 ? 0 : 1;
    ...
    const fullDay = [{startTime: DateTime.local().startOf('day').toISO(), endTime: DateTime.local().endOf('day').toISO()}];
    let timeRangesForDaysOfWeek = [fullDay, fullDay, fullDay, fullDay, fullDay, fullDay, fullDay];;

    if (timeRangeSpecified) {
      console.log('timeRangeSpecifiedd', timeRangeSpecified);
    ...
    if (descriptionJson && flexible) {
      ...
      if (!timeRangeSpecified && descriptionJson.timeRangesForDaysOfWeek.length > 0) {
    ...
    for (const attendeeGoogleEmail of attendeeGoogleEmails) {
      ...
      if (flexible) {
        ...
        let attendeeConstraints = timeRangesForDaysOfWeek;
        ...
        if (!timeRangeSpecified && category === 1 && descriptionJson && descriptionJson.timeRangesForDaysOfWeek.length < 1) {
          console.log('1');
