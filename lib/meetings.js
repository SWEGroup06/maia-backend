...
const context = {
   ...
   * @param {number} newDayOfWeek - integer coresponding to day of the week
   ...
   * @param {string} startDateRangeISO - DateTime ISO for the start date for the range given
   * @param {string} endDateRangeISO - DateTime ISO for the end date for the range given
   * @param {string} startTimeRangeISO - DateTime ISO for the start time for the range given
   * @param {string} endTimeRangeISO - DateTime ISO for the end time for the range given
   * @param {boolean} flexible - boolean to specifiy whether the event should be flexible between given range
   * @param {string} dayOfWeek - string coresponding to a day of the week or multiple days (weekend, weekday)
   * @param {boolean} timeRangeSpecified - boolean to specify whether the given time range is given or not
   * @return {Promise<void>}
   */
  async schedule(
    googleEmails,
    ...
    startDateRangeISO =
      DateTime.max(DateTime.fromISO(startDateRangeISO), oneHourFromNow).toISO();
    ...
    for (const email of googleEmails) {
      ...
      if (flexible && attendees_considered < MAX_ATTENDEES_CONSIDERED) {
        ...
        const minBreakLength = await this.getBreakLength(category, email);
   ...
   * @param {string} oldDateTimeISO - - DateTime ISO for the meeting to the rescheduled
   * @param {string} newStartDateRangeISO - DateTime ISO for the start date for the range given
   * @param {string} newEndDateRangeISO - DateTime ISO for the end date for the range given
   * @param {string} newStartTimeRangeISO - DateTime ISO for the start time for the range given
   * @param {string} newEndTimeRangeISO - DateTime ISO for the end time for the range given
   * @param {string} newDayOfWeek - string coresponding to a day of the week or multiple days (weekend, weekday)
   * @param {string} dateRangeSpecified - boolean to specify whether the given date range is given or not
   * @param {string} timeRangeSpecified - boolean to specify whether the given time range is given or not
   * @param {boolean} flexible - boolean value to specify if the event should be flexible
   * @return {Promise<*>} 
   */
  async reschedule(
    googleEmail,
    ...
    for (const attendeeGoogleEmail of attendeeGoogleEmails) {
      ...
      if (flexible && attendees_considered < MAX_ATTENDEES_CONSIDERED) {
        ...
        const minBreakLength = await this.getBreakLength(category, attendeeGoogleEmail);
  ...
  async getBreakLength(category, attendeeGoogleEmail) {
    ...
    if (category === 1 || category === -1) {
      ...
      const mins = await DATABASE.getMinBreakLength(attendeeGoogleEmail);
      console.log('min break length: ', mins);
      breakLength = Duration.fromObject({minutes: mins});
    }
    return breakLength;
  }
};
