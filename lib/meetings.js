...
const context = {
  ...
  async getTimeRangesForDaysOfWeek(timeRangeSpecified, timeRangesForDaysOfWeek, attendeeGoogleEmail, category,
                                   descriptionJson, newDayOfWeek) {
    console.log('---getTimeRangesForDaysOfWeek---');
    console.log(descriptionJson, descriptionJson.timeRangesForDaysOfWeek.length < 1)
    const fullDay = [
      {
    ...
    // working hours for work events
      ...
      if (newDayOfWeek.length > 0) {
        ...
        for (let day = 0; day < timeRangesForDaysOfWeek.length; day++) {
          if (timeRangesForDaysOfWeek[day].length === 0) newTimeRangesForDaysOfWeek.push([]);
          else {
      ...
      } else if (descriptionJson && descriptionJson.timeRangesForDaysOfWeek.length > 0) {
  ...
  async schedule(
    ...
    if (timeRangeSpecified || dayOfWeek.length > 0) {
      timeRangesForDaysOfWeek = TIME.generateTimeRangesForDaysOfWeek(
          dayOfWeek, startTimeRangeISO, endTimeRangeISO
      );
    }
    ...
    for (const email of googleEmails) {
      ...
      if (flexible) {
        ...
        const attendeeConstraints = await this.getTimeRangesForDaysOfWeek(timeRangeSpecified, timeRangesForDaysOfWeek,
            email, category, null, dayOfWeek);
  ...
  async reschedule(
    ...
    if (timeRangeSpecified || newDayOfWeek.length > 0) {
      console.log("timeRangeSpecified", timeRangeSpecified);
      timeRangesForDaysOfWeek = TIME.generateTimeRangesForDaysOfWeek(newDayOfWeek, newStartTimeRangeISO,
        newEndTimeRangeISO);
    }
    ...
    for (const attendeeGoogleEmail of attendeeGoogleEmails) {
      ...
      if (flexible) {
        ...
        let attendeeConstraints = await this.getTimeRangesForDaysOfWeek(timeRangeSpecified, timeRangesForDaysOfWeek,
            attendeeGoogleEmail, category, descriptionJson, newDayOfWeek);
