...
const context = {
   ...
   *
   * @param {boolean} timeRangeSpecified - whether or not the time range was specified by user
   * @param {string} dayOfWeek - the day of week if specified for an event to be on
   * @param {string} startTimeRangeISO - start of the range of times if specified
   * @param {string} endTimeRangeISO - end of the range of times if specified
   * @param {number} category - type of event
   * @return {Array|*[]} - time ranges to be considered for each day of the week
   */
  ...
  async getHistogram(category, token, email) {
    ...
    if (!histFreq) {
      throw new Error("Your Historical Data Cannot Be Found");
    }
    return histFreq;
  },

  /**
   * Schedule creates a new meeting with all the attendees, title, duration and
   * picks an optimal time slot using the scheduler. It returns this time slot.
   *
  ...
  /**
   * Reschedule takes the info associated with the old meeting and picks an
   * optimal slot for the new rescheduled meeting within a specified time range.
   *
  ...
  /**
   * generates the user histograms from their last 30 days of calendar data
   * and stores the histograms in the database
   *
   * @param {string} googleEmail is used to access their google calendar
   * @param {string} tokens is a string containing identification for the calendar
