...
const context = {
  ...
  async sp(
    ...
    const timeRangesForDaysOfWeek = TIME.generateTimeRangesForDaysOfWeek(
      dayOfWeek,
      startTimeRangeISO,
      endTimeRangeISO
    );
    ...
    for (const email of googleEmails) {
      ...
      if (flexible) {
        ...
        const fullDay = [
          {
            startTime: DateTime.local().startOf("day").toISO(),
            endTime: DateTime.local().endOf("day").toISO(),
          },
        ];
    ...
    for (const attendeeGoogleEmail of attendeeGoogleEmails) {
      ...
      if (flexible) {
        ...
        if (
          ...
          category === 1 &&
          descriptionJson &&
          descriptionJson.timeRangesForDaysOfWeek.length < 1
        ) {
          console.log("1");
          const workingHours = await DATABASE.getConstraintsFromGoogleEmail(
            attendeeGoogleEmail
