...
module.exports = {
  ...
  async schedule(title, slackEmails, startDateRange, endDateRange, flexible) {
    ...
    for (const email of slackEmails) {
      ...
      // console.log('now: ', today.toISO());
      // console.log('timestamp: ', DateTime.fromISO(histFreqInfo.timestamp).toISO());
      // console.log('diff: ', today.diff(DateTime.fromISO(histFreqInfo.timestamp), ['minutes']));
