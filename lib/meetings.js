...
const context = {
  ...
  async schedule(title, googleEmails, startDateRangeISO, endDateRangeISO, flexible, duration) {
    ...
    for (const email of googleEmails) {
      ...
      if (flexible) {
        ...
        const freeSlots = SCHEDULER.getFreeSlots((data.map((e) => [e.start, e.end])), startDateRangeISO,
            endDateRangeISO, minBreakLength, weekConstraints);
        if (data) freeTimes.push(freeSlots);
    ...
    const description = {
      ...
      // TODO: Add time ranges (Array) timeRangesForDaysOfWeek
    };
   ...
   * @return {Promise<void>}
   */
  async sp(googleEmails, title, duration, startDateRangeISO, endDateRangeISO, startTimeRangeISO, endTimeRangeISO,
      flexible, dayOfWeek) {
    const freeTimes = [];
    const constraints = [];
    const histFreqs = [];
    ...
    // TODO: AMELIA THIS IS WHAT U WANT
    // TODO: const timeRangesForDaysOfWeek = TIME.generateTimeRangesForDaysOfWeek(dayOfWeek, startTimeRangeISO,
    //  endTimeRangeISO);
    // TODO: AMELIA DO SOMETHING WITH THIS! ^
    ...
    for (const email of googleEmails) {
      ...
      if (flexible) {
        ...
        const data = await GOOGLE.getBusySchedule(token, startDateRangeISO, endDateRangeISO);
        if (data) {
          freeTimes.push(SCHEDULER.getFreeSlots((data.map((e) => [e.start, e.end])),
              startDateRangeISO, endDateRangeISO, minBreakLength));
        }
    ...
    if (flexible) {
      ...
      chosenSlot = {'start': startDateRangeISO, 'end': endDateRangeISO};
    }
    ...
    const description = {
      ...
      startDateRange: startDateRangeISO,
      endDateRange: endDateRangeISO,
      // TODO: timeRangesForDaysOfWeek: timeRangesForDaysOfWeek,
    };
  ...
  async tp(googleEmail, oldTitle, oldDateTimeISO, newStartDateRangeISO, newEndDateRangeISO,
      newStartTimeRangeISO, newEndTimeRangeISO, newDayOfWeek, timeRangeSpecified, dateRangeSpecified) {
    ...
    // Update description
    const prevDesc = this.getDescriptions([oldEvent])[0];
    console.log(prevDesc);
    if (prevDesc) {
      const description = JSON.parse(prevDesc.description);
    ...
    /**
     * TODO: This is the function we will use to generate time ranges in
     * TODO: the format [[], ..., [{'start': 12pm, 'end': 8pm}], [{'start': 12pm, 'end': 8pm}]]
     */
    ...
    if (prevDesc) {
      ...
      if (timeRangeSpecified) {
        timeRangesForDaysOfWeek = TIME.generateTimeRangesForDaysOfWeek(newDayOfWeek, newStartTimeRangeISO, newEndTimeRangeISO);
      } else {
        timeRangesForDaysOfWeek = prevDesc.timeRangesForDaysOfWeek;
      }
      if (!dateRangeSpecified) {
        newStartDateRangeISO = DateTime.max(DateTime.fromISO(prevDesc.startDateRange),
            DateTime.local().plus({hours: 2}).startOf('hour')).toISO();
        newEndDateRangeISO = DateTime.fromISO(prevDesc.endDateRange) > DateTime.local() ? prevDesc.endDateRange :
            newEndDateRangeISO;
      }
    }
    ...
    // TODO: Amelia + Hasan change this part (in for loop?)
    ...
    // TODO: Everything below is just copied from the old reschedule function
    const chosenSlot = await SCHEDULER.findMeetingSlot(freeTimes, eventDuration, null, histFreqs);
