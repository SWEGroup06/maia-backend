 ...
 */
   ...
   * @param {String} specificTimeGiven
   * @return {Promise<{start: string, end: string}|{msg: string}>}
   */
  async reschedule(eventStartTime, title, organiserSlackEmail, startOfRangeToRescheduleTo, endOfRangeToRescheduleTo, specificTimeGiven) {
    const constraints = [];
    ...
    if (!specificTimeGiven && description.length > 0 && description[0].flexible) {
      startOfRangeToRescheduleTo = description[0].startDateRange;
      endOfRangeToRescheduleTo = description[0].endDateRange;
    }

    description.startDateRange = startOfRangeToRescheduleTo;
    description.endDateRange = endOfRangeToRescheduleTo;
    // TODO: await GOOGLE.setDescription(organiserToken, originalEvent.id, JSON.stringify(description));
