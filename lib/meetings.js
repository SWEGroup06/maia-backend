...
const context = {
  ...
  async schedule(
    ...
    for (const email of googleEmails) {
      ...
      if (flexible) {
        ...
        // TODO: weekend dont consider workingHours... if (category === 1) {
        //   weekConstraints = await DATABASE.getConstraints(email);
        // }

        // Generate constraints in format the scheduler takes in
        const generatedConstraints = SCHEDULER.generateConstraints(
          weekConstraints,
          startDateRangeISO,
          endDateRangeISO
        );
        ...
        const data = await GOOGLE.getBusySchedule(
          token,
          startDateRangeISO,
          endDateRangeISO
        );
    ...
    const response = await GOOGLE.createMeeting(
      ...
      title !== ""
        ? title
        : `Meeting: ${DateTime.fromISO(chosenSlot.start).toLocaleString(
            DateTime.DATE_MED
          )}`,
      chosenSlot.start,
      chosenSlot.end,
  ...
  async reschedule(
    eventStartTimeISO,
    title,
    organiserGoogleEmail,
    startOfRangeToRescheduleToISO,
    endOfRangeToRescheduleToISO,
    specificTimeGiven
  ) {
    const constraints = [];
    const histFreqs = [];
    ...
    const organiserToken = JSON.parse(
      await DATABASE.getTokenFromGoogleEmail(organiserGoogleEmail)
    );
    ...
    if (
      ...
      events.length === 0 ||
      (eventStartTimeISO &&
        !TIME.compareTime(events[0].start.dateTime, eventStartTimeISO))
    ) {
      throw new Error("No event found to reschedule with given details");
    }
    ...
    const updatedDescription =
      descriptions.length > 0 ? JSON.parse(descriptions[0].description) : {};
    ...
    const freeTimes = [];
    const eventDuration = DateTime.fromISO(eventEndTime).diff(
      DateTime.fromISO(new Date(events[0].start.dateTime).toISOString())
    );
    ...
    for (const email of attendeeEmails) {
      ...
      // Retrieve user constraints in format: [{startTime: ISO Date/Time String, endTime: ISO Date/Time String}],
      const weekConstraints = await DATABASE.getConstraintsFromGoogleEmail(
        email
      );
    ...
    // Using free times find a meeting slot and get the choice
    const chosenSlot = await SCHEDULER.findMeetingSlot(
      freeTimes,
      eventDuration,
      constraints,
      histFreqs
