...
const context = {
    ...
    for (let i = 0; i < 7; i++) {
      if (days[i] === 1) {
        for (let j = 0; j < times.length; j++) {
          await DATABASE.setConstraintFromGoogleEmail(email,
  ...
  async getHistFreq(token, category, email) {
    let histFreqInfo = await DATABASE.getFrequenciesForCategoryFromGoogleEmail(email, category);
    let tried = 0;
    // TODO: Fix this wtf
    ...
    if (TODAY.diff(DateTime.fromISO(histFreqInfo.timestamp), ['minutes']) > HISTFREQLIFETIME) {
      ...
      await DATABASE.setFrequenciesForCategoryFromGoogleEmail(email, category, histFreq);
    }
    return histFreq;
  },
  /**
  ...
  async schedule(title, slackEmails, startDateRangeISO, endDateRangeISO, flexible, duration) {
    const freeTimes = [];
    const constraints = [];
    const histFreqs = [];
    let category;
    ...
    console.log('start: ', startDateRangeISO);
    console.log('end: ', endDateRangeISO);
   ...
   */
  async reschedule(eventStartTimeISO, title, organiserSlackEmail, startOfRangeToRescheduleToISO,
    ...
    const organiserToken = JSON.parse(await DATABASE.getTokenFromGoogleEmail(organiserSlackEmail));
    ...
    console.log('eventStartTimeISO******************');
    console.log(DateTime.fromISO(eventStartTimeISO).toISO());
    console.log('************************');
