 ...
 */
  ...
  reschedule: async function(eventStartTime, eventEndTime, newStartDateTime, newEndDateTime, organiserSlackEmail) {
    ...
    if (!newStartDateTime) {
      ...
      startOfRangeToRescheduleTo = DateTime.fromISO(JSON.parse(decodeURIComponent(newStartDateTime)));
    }

    let endOfRangeToRescheduleTo;
    if (!newEndDateTime) {
      endOfRangeToRescheduleTo = DateTime.local(startOfRangeToRescheduleTo.year, startOfRangeToRescheduleTo.month, startOfRangeToRescheduleTo.day + 14).toISO();
    } else {
      endOfRangeToRescheduleTo = DateTime.fromISO(JSON.parse(decodeURIComponent(newEndDateTime)));
    }
    ...
    try {
      ...
      const eventEndTime = new Date(events[0].end.dateTime).toISO();
      ...
      const busyTimes = [];
      const eventDuration = DateTime.fromISO(eventEndTime).diff(DateTime.fromISO(new Date(events[0].start.dateTime).toISO()));
