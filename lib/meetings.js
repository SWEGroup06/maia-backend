...
module.exports = {
  ...
  async schedule(title, slackEmails, startDateRange, endDateRange, flexible) {
    ...
    if (!chosenSlot) {
      ...
      return {
        msg: 'No meeting slot found please try a different day',
      };
    }

    const description = {flexible: flexible, category: category, startDateRange: startDateRange,
 ...
 */
  ...
  async reschedule(eventStartTime, title, organiserSlackEmail, startOfRangeToRescheduleTo, endOfRangeToRescheduleTo) {
    const constraints = [];
    ...
    const description = this.getDescriptions([originalEvent]);

    if (description.length > 0 && description[0].flexible) {
      if (startOfRangeToRescheduleTo == null) {
        startOfRangeToRescheduleTo = description[0].startDateRange;
      }
      if (endOfRangeToRescheduleTo == null) {
        endOfRangeToRescheduleTo = description[0].endDateRange;
      }
    } else {
      if (null == startOfRangeToRescheduleTo) {
        // only reschedule to at least 1 hour away from now
        startOfRangeToRescheduleTo = today.plus(Duration.fromObject({hours: 1}));
      }
      if (null == endOfRangeToRescheduleTo) {
        endOfRangeToRescheduleTo = today.plus(Duration.fromObject({days: 7}));
      }
    }
    ...
    for (const email of attendeeEmails) {
      ...
      const histFreqInfo = await DATABASE.getFrequenciesForCategoryFromID(email, category);
      let histFreq = histFreqInfo.histFreq;
      if (!histFreq || !histFreqInfo.timestamp || today.diff(DateTime.fromISO(histFreqInfo.timestamp), ['minutes']) > histFreqLifetime) {
        // if out of date, recalculate hist freq and update in db
        const lastMonthHist = await GOOGLE.getMeetings(token, oneMonthAgo.toISO(), today.toISO());
        histFreq = await SCHEDULER.getUserHistory(lastMonthHist.map((e) => [e.start.dateTime, e.end.dateTime, e.summary]), category);
        await DATABASE.setFrequenciesByCategory(email, category, histFreq);
