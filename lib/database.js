...
module.exports = {
  ...
  createNewUser: function(userID, slackEmail, googleEmail, googleAuthToken) {
    ...
    const user = new User({
      ...
      frequencies: [initialiseConstraints()],
    });
   ...
   * @param {String} userID
   */
  getFrequenciesForCategoryFromID: async function(userID, category) {
    try {
      const user = await User.findOne({id: userID});
   ...
   * TODO: Comment
   * @param {String} userID
   * @param {Number} category - index coresponding to correct event category
   * @param {Number} frequency - new frequency table to set at the given index

   */
  setFrequenciesByCategory: async function(userID, category, frequencyTable) {

    await User.findOneAndUpdate(
        {'id': userID},
        {$set: {
          [`frequencies.${category}`]: frequencyTable,
        }},
    );
  },
