...
module.exports = {
  ...
  getFrequenciesForCategoryFromID: async function(userID, category) {
    try {
      const user = await User.findOne({id: userID});
      return user ? user.frequencies[category + 1] : null;
    } catch (err) {
      console.error(err);
      return null;
    }
   ...
   * @param {Number} frequencyTable - new frequency table to set at the given index
   */
  setFrequenciesByCategory: async function(userID, category, frequencyTable) {
    try {
      await User.findOneAndUpdate(
          {'id': userID},
          {$set: {
            [`frequencies.${category + 1}`]: frequencyTable,
          }},
      );
    } catch (err) {
      console.log(err);
