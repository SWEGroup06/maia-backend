...
module.exports = {
  ...
  createNewUser: function(userID, slackEmail, googleEmail, googleAuthToken) {
    ...
    function initialiseFrequencies() {
      return [{histFreq: [], timestamp: null}, {histFreq: [], timestamp: null}, {histFreq: [], timestamp: null},
        {histFreq: [], timestamp: null}, {histFreq: [], timestamp: null}, {histFreq: [], timestamp: null}, {histFreq: [], timestamp: null}];
    }
  ...
  setFrequenciesByCategory: async function(email, category, frequencyTable) {
    try {
      await User.findOneAndUpdate(
          {'email': email},
          {$set: {
            [`frequencies.${category + 1}.histFreq`]: frequencyTable,
            [`frequencies.${category + 1}.timestamp`]: new Date().toISOString(),
          }},
      );
    } catch (err) {
      console.log(err);
