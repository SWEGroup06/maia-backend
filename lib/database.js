...
const context = {
   ...
   * @return {boolean}  - success is true or false depending on whether the creation of the new user succeeded or not
   */
  createNewUser: function (googleEmail, googleTokens, slackEmail, slackId) {
     ...
     *
     * @return {Array} - {({histFreq: [[]], timestamp: null})[]} each of the 7 arrays is for a given day in the week
     */
  ...
  /**
   * Given a google email, sets the frequency table for the given event category to the new frequency table passed in as a a parameter
   *
   * @param {string} email - The user's Google email address
  ...
  /**
   * Set the users working hours for every day given their google email and the new array
   *
   ...
   * @return {Promise<void>} - Return the auto rescheduling interval stored for the user
   */
