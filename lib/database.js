...
=======
      ...
      console.log('Connection to DB Successful');
>>>>>>> b2cab58fb05c58a4bea8259e6f29c0b84d08b4e1
    } catch (error) {
  ...
  createNewUser: function (googleEmail, googleTokens, slackEmail, slackId) {
    ...
    const doc = {
      ...
      schedulingPreferences: {minBreakLength: 0, autoReschedulingInterval: null, clustering: true},
    };
   ...
   */
<<<<<<< HEAD
  setConstraintFromGoogleEmail: async function (
    email,
    startTime,
    endTime,
    dayOfWeek
...
=======
  setConstraintForDayFromGoogleEmail: async function(email, startTime, endTime, dayOfWeek) {
>>>>>>> b2cab58fb05c58a4bea8259e6f29c0b84d08b4e1
...
=======
   ...
   * @return {Promise<void>}
   */
  setAutoReschedulingInterval: async function(email, minAutoReschedulingInterval) {
    try {
      await User.findOneAndUpdate(
          {'google.email': email},
          {$set: {[`schedulingPreferences.autoReschedulingInterval`]: minAutoReschedulingInterval}},
      );
    } catch (err) {
      console.log(err);
    }
