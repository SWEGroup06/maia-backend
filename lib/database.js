...
module.exports = {
  ...
  createNewUser: function(userID, slackEmail, googleEmail, googleAuthToken) {
    ...
    function initialiseConstraints() {
      const defaultWorkHours = [{'startTime': DateTime.fromObject({hour: 0}).startOf('day').toISO(), 'endTime': DateTime.fromObject({hour: 1}).toISO()},
        {'startTime': DateTime.fromObject({hour: 6}).toISO(), 'endTime': DateTime.fromObject({hour: 24}).endOf('day').toISO()}];
      return [defaultWorkHours, defaultWorkHours, defaultWorkHours, defaultWorkHours, defaultWorkHours, defaultWorkHours, defaultWorkHours];
    }
   ...
   * @param {String} userID
   */
  getFrequenciesFromID: async function(userID) {
    try {
      const user = await User.findOne({id: userID});
  ...
  getFrequenciesForCategoryFromID: async function(email, category) {
    try {
      const user = await User.findOne({$or: [{'email': email}, {'google.email': email.toLowerCase()}]});
   ...
   */
  updateFrequencies: async function(userID, frequencies) {
    try {
  ...
  updateToken: async function(email, newToken) {
    if (await this.tokenExists(newToken)) {
      ...
      User.findOneAndUpdate({$or: [{'email': email}, {'google.email': email.toLowerCase()}]},
