...
const context = {
   ...
   *
   * @param {boolean} hideLogs (Optional) hide logging
   * @return {Promise<void>}
   */
  getDatabaseConnection: async function (hideLogs) {
    try {
      ...
      if (!hideLogs) console.log("DATABASE Connected");
    } catch (err) {
      if (!hideLogs) console.error("getDatabaseConnection Error:", err.message);
    }
  },
  ...
  createNewUser: async function (
    ...
    if (slackEmail && slackId) {
      ...
      if (await context.userExists(googleEmail)) {
  ...
  /**
   * Checks if the all users provided exist
   *
   * @param {string} emails - The user's Google or Slack email addresses
   * @return {Array|null} Returns null if all logged in otherwise a list of the users not logged in
   */
  allUsersExists: async function (emails) {
    try {
      const notLoggedIn = [];
      for (const email of emails) {
        if (!(await context.userExists(email))) {
          notLoggedIn.push(email);
        }
      }
      return notLoggedIn.length ? notLoggedIn : null;
    } catch (err) {
      console.error("allUsersExists Error:", err.message);
      return null;
    }
  },
  ...
  getGoogleEmailsFromSlackEmails: async function (emails) {
    try {
      ...
      const slackGoogleMapping = {};
      for (const user of users) {
        if (user && user.slack && user.slack.email) {
          slackGoogleMapping[user.slack.email.toLowerCase()] =
            user.google.email;
        }
      }
      ...
      const googleEmails = [];
      for (const email of emails) {
        const googleEmail = slackGoogleMapping[email.toLowerCase()];
        if (googleEmail) {
          googleEmails.push(googleEmail);
        }
      }
  ...
  /**
   * Given a google email, sets the frequency table for the all categories
   * to the new frequency table passed in as a a parameter
   *
   * @param {string} email - The user's Google email address
   * @param {Array} frequencyTables - a list of new frequency tables
   */
  setFrequenciesForAllCategoriesFromGoogleEmail: async function (
    email,
    frequencyTables
  ) {
    try {
      ...
      await User.findOneAndUpdate(
        ...
        {
          $set: {
            frequencies: frequencyTables.map((table) => {
              return {
                histFreq: table,
                timestamp: now,
              };
      ...
      console.error(
        "setFrequenciesForAllCategoriesFromGoogleEmail Error:",
        err.message
      );
    }
  },
   ...
   * @param {Array} constraints - fixed 2D array of size 7, in format:  [
   * [{startTime, endTime}], …]
   * @return {Promise<void>} - no clue
   */
  setConstraintsGivenFullArray: async function (email, constraints) {
    try {
      await User.findOneAndUpdate(
        { "google.email": email },
