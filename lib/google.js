...
module.exports = {
  ...
  getTokenFromGoogleEmails(code) {
    return new Promise(function(resolve, reject) {
      oauth2Client.getTokenFromGoogleEmail(code, function(err, tokens) {
        if (err) {
          reject(err);
          return;
        }
   ...
   * @return {Promise<Google Calendar Event>}
   */
  getEventByName(tokens, eventName, startRangeToReschedule = TODAY.toISO(),
