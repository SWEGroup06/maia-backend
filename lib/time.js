...
module.exports = {
  ...
  generateTimeRangeFromGoogleEvent(oldEvent, newStartTimeRangeISO, newEndTimeRangeISO) {
    ...
    const duration = oldEventStart.diff(oldEventEnd, 'minutes');

    const start = DateTime.fromISO(newStartTimeRangeISO);
  ...
  generateTimeRangesForDaysOfWeek(dayOfWeek, startTimeRangeISO, endTimeRangeISO) {
    ...
    if (dayIndex !== -1) {
      busyDays[dayIndex] = [{start: startTimeRangeISO, end: endTimeRangeISO}];
      return busyDays;
    }

    if (dayOfWeek === 'Weekend') {
      for (let day = SATURDAY; day <= SUNDAY; day++) {
        busyDays[day] = [{start: startTimeRangeISO, end: endTimeRangeISO}];
      }
      return busyDays;
    }
